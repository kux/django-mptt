...
class GenreCreatingThread(threading.Thread):

    def __init__(self, thread_no, run_in_transaction, select_all_before):
        super(GenreCreatingThread, self).__init__()
        self.thread_no = thread_no
        self.run_in_transaction = run_in_transaction
        self.select_all_before = select_all_before

    def _run(self):
        if self.select_all_before:
            # useful for testing REPEATEABLE-READ behavior
            len(Genre.objects.all())
        for i in xrange(10):
            name = str(self.thread_no * 10 + i)
...
class ConcurrencyTests(TreeTestCase):

    def _run_threads(self, thread_class, run_in_transaction, select_all_before):
        threads = []
        for thread_no in xrange(15):
            new_thread = thread_class(thread_no, run_in_transaction, select_all_before)
            new_thread.start()
    ...
    def test_concurrent_inserts_no_wrapping_transaction(self):
        self._run_threads(GenreCreatingThread, False, True)
        genres = Genre.objects.all()
        self.assertEqual(len(set(genre.tree_id for genre in genres)), len(genres))

    @skipUnlessDBFeature('test_db_allows_multiple_connections')
    def test_concurrent_inserts_wrapping_transaction(self):
        self._run_threads(GenreCreatingThread, True, True)
        genres = Genre.objects.all()
        self.assertEqual(len(set(genre.tree_id for genre in genres)), len(genres))
